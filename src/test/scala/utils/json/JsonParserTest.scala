...
class JsonParserTest extends AnyFunSuite{
  ...
  val longUrl = "https://www.sueddeutsche.de/panorama/fabrikbrand-delhi-indien-1.4714938"

  test("parseNewsArticle should parse NewsArticle with all fields"){
    val parsedJson = JsonParser.parseNewsArticle(json)
    assert(parsedJson.id === id)
    assert(parsedJson.title === title)
    assert(parsedJson.intro === intro)
    assert(parsedJson.text === text)
    assert(parsedJson.longUrl === longUrl)
    assert(parsedJson.crawlTime === crawlTime)
  }
}
